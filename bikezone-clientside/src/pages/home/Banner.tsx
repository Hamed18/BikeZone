...
const Banner: React.FC<BannerProps> = ({ banners = defaultBanners }) => {
  ...
  return (
    <div className="relative w-full">
      <Swiper {...swiperParams} className="h-[50vh] md:h-[70vh]">
        {banners.map((banner) => (
            ...
            <div className="absolute inset-0 backdrop-brightness-75 bg-black/20 flex items-center justify-center">
              <div className="text-center text-white px-4 max-w-4xl mx-auto">
                ...
                {banner.ctaText && (
                  <Link
                    to={`/products`}
                    className="bg-primary hover:bg-primary-dark text-white dark:text-black font-bold py-2 px-6 rounded-md text-lg animate-fadeIn delay-200"
                  >
                    {banner.ctaText}
                  </Link>
                )}
